(function () {
  ...
  (function () {
    ...
    g.Time.prototype.update = function () {
      ...
      this.ndelta = Math.min(
        Math.max(this.delta / (1000 / 60), 0.0001),
        10
      );
    ...
    g.FoodTile.prototype.update = function () {
      ...
      this.blur =
        ...
        Math.sin(this.parentState.time.elapsed / 200) *
          ...
          0.015;
      this.scale =
        0.8 + Math.sin(this.parentState.time.elapsed / 200) * 0.2;

      if (
        this.parentState.food.birthTick ||
        this.parentState.food.deathTick
      ) {
    ...
    g.Snake.prototype.update = function () {
      ...
      if (this.updateTick >= this.updateTickMax) {
        ...
        if (
          ...
          this.parentState.stageElem.removeChild(
            this.parentState.food.tile.elem
          );
